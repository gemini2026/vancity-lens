...
| Stage 6 - Rollout | MIG-015A | Draft staging cutover runbook + remote validation mode | DONE | Runbook exists; validation script supports remote API validation without local pytest | Confirm `migration/staging-cutover-runbook.md` exists; run `bash migration/validate_k2_migration.sh --help` shows `--skip-pytest` |

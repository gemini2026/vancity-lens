...
async def discover_dpb_minutes(session: aiohttp.ClientSession) -> List[Dict]:
    ...
    try:
        async with session.get(
            DPB_MINUTES_URL, headers=HEADERS, timeout=aiohttp.ClientTimeout(total=30)
        ) as resp:
            if resp.status != 200:
...
async def download_and_store(
    db_pool: asyncpg.Pool, max_documents: Optional[int] = None
) -> Dict:
    """
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        try:
            ...
            for idx, doc_info in enumerate(documents, 1):
                try:
                    logger.info(
                        f"Processing document {idx}/{len(documents)}: {doc_info['filename']}"
                    )
                    ...
                    stats["errors"].append(
                        f"Processing error {doc_info['filename']}: {str(e)}"
                    )
                    continue

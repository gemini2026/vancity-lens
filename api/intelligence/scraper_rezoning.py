...
async def discover_applications(session: aiohttp.ClientSession) -> List[Tuple[str, str, str]]:
    """
    ...
    try:
        async with session.get(APPLICATIONS_URL, headers=HEADERS, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            if resp.status != 200:
            ...
            if not app_rows:
                logger.warning("No application rows found - checking alternative selectors")
            ...
            for row in app_rows:
                try:
                    ...
                    address_elem = row.select_one(".address, [data-address], .location, .title")
...
async def scrape_application_page(session: aiohttp.ClientSession, url: str) -> Dict:
    ...
    try:
        async with session.get(url, headers=HEADERS, timeout=aiohttp.ClientTimeout(total=30)) as resp:
            if resp.status != 200:
                logger.error(f"Failed to fetch application page {url}: HTTP {resp.status}")
            ...
            result = {
                ...
                'current_zoning': extract_text_by_label(soup, 'current zoning', 'existing zoning'),
                'proposed_zoning': extract_text_by_label(soup, 'proposed zoning', 'new zoning'),
                'description': extract_text_by_label(soup, 'description', 'summary', 'project description'),
    ...
    async with aiohttp.ClientSession(connector=connector, timeout=timeout) as session:
        try:
            ...
            for idx, (app_url, address, status) in enumerate(applications, 1):
                try:
                    logger.info(f"Processing application {idx}/{len(applications)}: {address}")
                    
                    # Scrape application page
                    app_data = await scrape_application_page(session, app_url)
                    
                    if 'error' in app_data:
                        stats['errors'].append(f"Failed to scrape {app_url}: {app_data['error']}")
                        continue
                    ...
                    for pdf_url in pdf_links:
                        try:
                            ...
                            stats['errors'].append(f"PDF processing error {pdf_url}: {str(e)}")
                            continue
                    ...
                    stats['errors'].append(f"Application processing error {app_url}: {str(e)}")
                    continue
    ...
    try:
        async with db_pool.acquire() as conn:
            ...
            doc_id = await conn.fetchval(
                ...
                RETURNING id
                """,
                url, source_type, content, content_type, metadata or {}
            )
...
def extract_text_by_label(soup: BeautifulSoup, *labels: str) -> str:
    ...
    for label in labels_lower:
        ...
        for div in soup.find_all("div"):
            ...
            if label in (div_class + [div_data] if isinstance(div_class, list) else [str(div_class), div_data]):

...
class ComparableSale(BaseModel):
    ...
    distance_m: Optional[Decimal] = Field(
        None, description="Distance in meters from query point"
    )
...
class ComparableSalesService:
        ...
        async with pool.acquire() as conn:
            ...
            if same_zoning and zoning:
                ...
                rows = await conn.fetch(
                    query, geom_text, radius_m, zoning, cutoff_date, limit
                )
            else:
            ...
            comparables = [
                ComparableSale(
                    ...
                    if isinstance(row["sale_date"], datetime)
    ...
    async def ingest_sales_data(
        pool: asyncpg.Pool, records: List[Dict[str, Any]]
    ) -> Dict[str, int]:
        """

...
def _compute_three_scenarios(
    ...
    items = [
        HiddenCostItem(category=cat, cost=cost, explanation=expl)
...
def _generate_gap_analysis(
    ...
    if not parts:
        return (
            f"Bull case alpha: {_fmt(bull_alpha)}. Base case alpha: {_fmt(base_alpha)}."
        )
...
def _calculate_execution_difficulty(
    ...
    if active_licence_count > 0:
        ...
        factors.append(
            f"{active_licence_count} existing tenant(s) (+{min(2, active_licence_count)})"
        )
    ...
    if neighborhood and entitled_storeys >= 13:
        ...
        is_soft = any(
            z.upper() in n_upper or n_upper in z.upper() for z in SOFT_SOIL_ZONES
        )
        if is_soft:
            score += 1
    ...
    has_opposition = any(
        f.code in ("HIGH_OPPOSITION", "MODERATE_OPPOSITION") for f in risk_flags
    )
    if has_opposition:
        score += 1
    ...
    try:
        ...
        if tree_count and tree_count > 0:
            ...
            if tree_points > 0:
                score += tree_points
                factors.append(
                    f"{tree_count} protected tree(s) within 50m (+{tree_points})"
                )
    except Exception:
        pass
...
async def compute_validation(
    ...
    try:
        ...
        if view_cone:
            cone_name = (
                view_cone["view_cone_name"]
                or view_cone["description"]
                or "Protected View"
            )
            risk_flags.append(
                RiskFlag(
        ...
        if nearby_permits and nearby_permits > 0:
            ...
            risk_flags.append(
                RiskFlag(
                    ...
                    cost_impact="Potential absorption delay: 3-12 months"
                    if nearby_permits >= 3
                    else None,
    ...
    try:
        ...
        nearby_business_types = [
            r["business_type"] for r in licences if r["business_type"]
        ]
        ...
        enviro_businesses = [
            bt
            for bt in nearby_business_types
    ...
    if best and lot_area:
        ...
        current_zoning = (
            parcel.get("current_zoning", None) if hasattr(parcel, "get") else None
        )
        try:
            current_zoning = parcel["current_zoning"]
        except (KeyError, TypeError):
...
def _build_one_liner(
    grade, friction, flags, three_scenario, best, neighborhood, exec_score
) -> str:
    ...
    if grade == "F":
        ...
        if three_scenario:
            ...
            base_alpha = "N/A"
        positives = (
            ", ".join(g.label for g in greens[:2]) if greens else "clean profile"
        )

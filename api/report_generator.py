...
class ReportGenerator:
    ...
    async def _fetch_parcel_data(
        ...
        async with db_pool.acquire() as conn:
            ...
            try:
                ...
                if heritage_row and heritage_row["category"]:
                    data.heritage_designation = heritage_row["category"]
            except Exception as e:
                logger.debug("Heritage designation query failed for %s: %s", pid, e)

            # Parcel-level contamination check (~50m from parcel centroid)
            try:
                contam_row = await conn.fetchrow(
                    """
                    SELECT cs.status
                ...
                if contam_row and contam_row["status"]:
                    data.contamination_status = contam_row["status"]
            except Exception as e:
                logger.debug("Contamination status query failed for %s: %s", pid, e)
    ...
    def _build_sources(self, pdf: FPDF, parcel_data: ParcelReport):
        ...
        methodology_lines = [
            ...
            "centroids with transit station buffers (200m/400m/800m). Distance measured via geodesic calculation (WGS84 geography).",
